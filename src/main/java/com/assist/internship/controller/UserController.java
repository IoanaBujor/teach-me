...
public class UserController {
    ...
    public ResponseEntity delete(@RequestBody User user, @RequestHeader("reset_token") final String token) {
        if (user.getActive() == 1) {
            if (userService.isAdmin1(token)) {
                ...
                if (newuser != null) {
                    ...
                    return ResponseEntity.status(HttpStatus.OK).body(new InternshipResponse(true, "User was deleted successfully!", Arrays.asList(newuser)));
                } else {
